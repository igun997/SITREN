...
<html lang="en">
  ...
  <body>
    <h3 align="center">Laporan Penapatan Biaya Admin </h3>
    <h3 align="center">Periode [{{date("d-m-Y",strtotime($req->input("start")))}} - {{date("d-m-Y",strtotime($req->input("end")))}}]</h3>
      <p><b>Pendapatan Periode Ini : </b>{{(\Sitren\TransaksiModel::orderBy("created_at","desc")->where(["jenis"=>"biaya_admin"])->whereBetween("created_at",[$req->input("start"),$req->input("end")]))->sum("jumlah")}}</p>
      <table >
          <thead>
            <tr>
              <th>No</th>
          ...
          <tbody>
            @foreach((\Sitren\TransaksiModel::orderBy("created_at","desc")->where(["jenis"=>"biaya_admin"])->whereBetween("created_at",[$req->input("start"),$req->input("end")]))->get() as $ks => $vs)
            <tr>
              <td>{{$ks+1}}</td>
